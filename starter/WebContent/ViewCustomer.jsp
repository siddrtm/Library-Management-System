...
<%-- 				    <% resultset.next(); %> --%>
		 ...
		 <div id="templatemo_content_center">
        	<div >
			        ...
			        <%
				    resultset = (ResultSet) session.getAttribute("requestedInfo");
...
<%-- 				    <% resultset.next(); %> --%>
				...
				<%  ResultSet rs_issue = (ResultSet) request.getAttribute("rs_issue");
					long sum =0;				
				%>
				 <TABLE style="position: absolute;left: 450px;top: 550px">
				    <TR>
				        <TH>Media_ID</TH>
				        <TH>Title</TH>
				        <TH>Issue Date</TH>
				    ...
				    <% while(rs_issue.next()){ %>
				    <TR>
				        <TD> <%= rs_issue.getString(1) %></TD>
				        <TD> <%= rs_issue.getString(3) %></TD>
				        <TD> <%= rs_issue.getString(4) %></TD>
				        <TD> <%= rs_issue.getString(5) %></TD>
				        <TD> <%= rs_issue.getString(6) %></TD>
				        <TD> <%= rs_issue.getString(7) %></TD>
				        <TD> <%= rs_issue.getString(8) %></TD>
				        <TD> <%= fine.calcFine(rs_issue.getString(5)) %></TD>
				       
				    </TR>
				    <% sum+= fine.calcFine(rs_issue.getString(5));} %>
